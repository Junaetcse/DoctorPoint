...
public class NeuromedicineListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_neuromedicine_list);

        Intent intent = getIntent();
        id=intent.getStringExtra("id");
        pdate=intent.getStringExtra("date");

        pd = new ProgressDialog(NeuromedicineListActivity.this);
        pd.setMessage("loading");
        pd.setCancelable(false);
        pd.setCanceledOnTouchOutside(false);
        String url= "http://techborno.com/Final_Project/Neuromedicine/search.php?appoint_date="+pdate+"&"+"doctor_id="+id;
        pd.show();
        listView = (ListView) findViewById(R.id.neuromedicine_listview);
