...
public class UrologyLIstActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_urology_list);

        Intent intent = getIntent();
        id=intent.getStringExtra("id");
        pdate=intent.getStringExtra("date");

        pd = new ProgressDialog(UrologyLIstActivity.this);
        pd.setMessage("loading");
        pd.setCancelable(false);
        pd.setCanceledOnTouchOutside(false);
        String url= "http://techborno.com/Final_Project/Urologist/search.php?appoint_date="+pdate+"&"+"doctor_id="+id;
        pd.show();
        listView = (ListView) findViewById(R.id.urology_listview);
